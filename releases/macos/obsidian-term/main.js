...
var require_xterm = __commonJS({
  "node_modules/xterm/lib/xterm.js"(exports2, module2) {
    !function(e, t) {
...
var require_xterm_addon_fit = __commonJS({
  "node_modules/xterm-addon-fit/lib/xterm-addon-fit.js"(exports2, module2) {
    !function(e, t) {
...
var VaultScanner = class {
   ...
   * 递归扫描文件夹
   */
  scanFolder(folder, items, vaultPath) {
    for (const child of folder.children) {
      if (child.name.startsWith(".")) {
        continue;
      }
      const isFolder = child instanceof import_obsidian.TFolder;
      ...
      if (isFolder) {
        this.scanFolder(child, items, vaultPath);
      }
    }
  }
  /**
...
var AutocompleteManager = class {
  constructor(scanner, container, onSelect) {
    this.popup = null;
   ...
   * 处理输入字符
   */
  handleInput(char, cursorPosition) {
  ...
  renderPopup() {
    ...
    this.popup.innerHTML = this.state.items.map((item, index) => {
...
var TERMINAL_VIEW_TYPE = "xterm-terminal-view";
var TerminalView = class extends import_obsidian2.ItemView {
  constructor(leaf, plugin) {
    ...
    this.lastResizeCols = null;
    this.lastResizeRows = null;
    this.inputBuffer = "";
  ...
  getPtyHelperPath() {
    ...
    const pluginDir = manifestDir ? path.isAbsolute(manifestDir) ? manifestDir : path.join(vaultPath, manifestDir) : path.join(vaultPath, ".obsidian", "plugins", this.plugin.manifest.id);
    return path.join(pluginDir, "resources", "pty-helper");
  }
  /**
   * Start the PTY helper process
   */
  async startPtyHelper() {
    ...
    try {
      ...
      this.ptyProcess = (0, import_child_process.spawn)(ptyHelperPath, [userShell], {
        cwd: this.scanner.getVaultPath(),
        env: {
          ...process.env,
          TERM: "xterm-256color",
          LANG: process.env.LANG || "en_US.UTF-8",
          XTERM_INITIAL_COLS: String((_b = (_a = this.terminal) == null ? void 0 : _a.cols) != null ? _b : 0),
          XTERM_INITIAL_ROWS: String((_d = (_c = this.terminal) == null ? void 0 : _c.rows) != null ? _d : 0)
        },
        stdio: ["pipe", "pipe", "pipe", "pipe", "pipe"]
        // stdin, stdout, stderr, resize fd, status fd
      });
      this.lastResizeCols = (_f = (_e = this.terminal) == null ? void 0 : _e.cols) != null ? _f : null;
      this.lastResizeRows = (_h = (_g = this.terminal) == null ? void 0 : _g.rows) != null ? _h : null;
      if (this.ptyProcess.stdio && this.ptyProcess.stdio[3]) {
        this.resizePipe = this.ptyProcess.stdio[3];
      }
      if (this.ptyProcess.stdio && this.ptyProcess.stdio[4]) {
        const statusPipe = this.ptyProcess.stdio[4];
        statusPipe.on("data", (data) => {
          this.handleForegroundStatusChunk(data.toString());
        });
      }
      (_i = this.ptyProcess.stdout) == null ? void 0 : _i.on("data", (data) => {
        var _a2;
        const output = this.sanitizeTerminalOutput(data.toString("utf-8"));
        if (output) {
          (_a2 = this.terminal) == null ? void 0 : _a2.write(output);
        }
      });
      (_j = this.ptyProcess.stderr) == null ? void 0 : _j.on("data", (data) => {
        var _a2;
        const output = this.sanitizeTerminalOutput(data.toString("utf-8"));
        if (output) {
          (_a2 = this.terminal) == null ? void 0 : _a2.write(`\x1B[31m${output}\x1B[0m`);
        }
      });
      this.ptyProcess.on("close", (code) => {
        var _a2, _b2;
        (_a2 = this.terminal) == null ? void 0 : _a2.writeln(`\r
\x1B[90m[Shell exited with code ${code}]\x1B[0m`);
        (_b2 = this.terminal) == null ? void 0 : _b2.writeln("\x1B[90mPress any key to restart...\x1B[0m");
        this.ptyProcess = null;
        this.foregroundCommands = [];
      });
...
\x1B[31mError: ${err.message}\x1B[0m`);
        (_b2 = this.terminal) == null ? void 0 : _b2.writeln("\x1B[33mMake sure the bundled PTY helper is built and deployed.\x1B[0m");
        this.ptyProcess = null;
        this.foregroundCommands = [];
      });
      setTimeout(() => this.sendResize(), 100);
    } catch (error) {
      (_k = this.terminal) == null ? void 0 : _k.writeln(`\x1B[31mFailed to start terminal: ${error.message}\x1B[0m`);
      (_l = this.terminal) == null ? void 0 : _l.writeln("\x1B[33mMake sure the Rust PTY helper is built and deployed.\x1B[0m");
    }
  }
  /**
   * Send terminal size to PTY helper via fd 3
   */
  sendResize(cols = ((_a) => (_a = this.terminal) == null ? void 0 : _a.cols)(), rows = ((_b) => (_b = this.terminal) == null ? void 0 : _b.rows)()) {
    if (!this.terminal || !this.resizePipe || !cols || !rows)
      return;
  ...
  async handlePasteEvent(event) {
    ...
    if (!clipboardData || !this.terminal)
      return;
    ...
    event.preventDefault();
    event.stopPropagation();
    (_a = this.autocomplete) == null ? void 0 : _a.deactivate();
    this.inputBuffer = text.includes("\n") || text.includes("\r") ? "" : this.inputBuffer + text;
    this.terminal.paste(text);
  }
  async handleImagePaste(imageFile) {
    var _a, _b;
    try {
      ...
      (_b = this.terminal) == null ? void 0 : _b.paste(pastedPath);
      new import_obsidian2.Notice(`Image saved to ${filePath}`, 3e3);
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
      new import_obsidian2.Notice(`Failed to paste image: ${message}`, 4e3);
    }
  }
  getClipboardImageExtension(mimeType) {
    switch (mimeType) {
      case "image/jpeg":
        return "jpg";
      case "image/gif":
  ...
  findIncompleteAnsiSequenceStart(output) {
  ...
  stripBackgroundColors(output) {
    return output.replace(/\x1b\[([0-9;]*)m/g, (fullMatch, rawParams) => {
      const filteredParams = this.filterSgrParams(rawParams);
      return filteredParams === null ? "" : `\x1B[${filteredParams}m`;
    });
  }
  filterSgrParams(rawParams) {
    if (rawParams === "") {
      return rawParams;
    }
  ...
  handleInput(data) {
    ...
    if ((_a = this.autocomplete) == null ? void 0 : _a.isActive()) {
      if (data === "\x1B[A") {
    ...
    if (this.shouldActivateAutocomplete(data, trigger)) {
      this.inputBuffer += data;
      this.writeToPty(data);
      (_b = this.autocomplete) == null ? void 0 : _b.activate(this.inputBuffer.length);
      return;
    }
    if (data === "\r" || data === "\n") {
      this.inputBuffer = "";
   ...
   */
  handleAutocompleteSelect(absolutePath, searchText) {
    var _a;
    if (!((_a = this.ptyProcess) == null ? void 0 : _a.stdin))
      return;
  ...
  cleanup() {
    ...
    (_b = this.autocomplete) == null ? void 0 : _b.deactivate();
  }
  async onClose() {
    this.cleanup();
  }
  ...
  async ensureTerminalFontReady(fontFamily, fontSize) {
  ...
  getTerminalTypography(fontFamily, fontSize) {
  ...
  shouldActivateAutocomplete(data, trigger) {
    if (!trigger)
      return false;
    if (data.length !== 1)
      return false;
    if (data.charCodeAt(0) < 32)
      return false;
    if (trigger === "@" && this.isAgentCliInForeground())
      return false;
    const nextBuffer = this.inputBuffer + data;
    return nextBuffer.endsWith(trigger);
  }
  handleForegroundStatusChunk(chunk) {
    var _a;
    this.foregroundStatusBuffer += chunk;
    const lines = this.foregroundStatusBuffer.split("\n");
    this.foregroundStatusBuffer = (_a = lines.pop()) != null ? _a : "";
    for (const line of lines) {
      if (!line.startsWith("foreground"))
        continue;
      const commands = line.split("	").slice(1).map((command) => command.trim()).filter((command) => command.length > 0);
      this.foregroundCommands = commands;
    }
  }
  isAgentCliInForeground() {
    return this.foregroundCommands.some((command) => this.matchesAgentCli(command));
  }
  matchesAgentCli(command) {
    const normalized = command.toLowerCase();
    return normalized.includes("claude") || normalized.includes("codex") || normalized.includes("gemini");
  }
  getVisibleCursorTheme(theme) {
    const fallbackCursor = this.getHighContrastColor(theme.background);
    const resolvedCursor = this.isCursorVisible(theme.cursor, theme.background) ? theme.cursor : fallbackCursor;
    const resolvedCursorAccent = this.isCursorVisible(theme.cursorAccent, resolvedCursor) ? theme.cursorAccent : theme.background;
    return {
      ...theme,
  ...
  isCursorVisible(cursorColor, backgroundColor) {
...
var FALLBACK_THEME_NAME = "tokyo-night-storm";
var FALLBACK_THEME_CONTENT = `background = #24283b
foreground = #c0caf5
...
function parseGhosttyTheme(name, content) {
  return parseGhosttyThemeWithFallback(name, content, getParsedFallbackTheme());
}
async function readGhosttyThemeDirectory(themeDir, idPrefix) {
  try {
    const entries = await fs2.promises.readdir(themeDir, { withFileTypes: true });
    const files = entries.filter((entry) => entry.isFile() && !entry.name.startsWith(".")).sort((a, b) => a.name.localeCompare(b.name));
    const themes = [];
    for (const entry of files) {
      const filePath = path2.join(themeDir, entry.name);
...
function parseGhosttyThemeWithFallback(name, content, fallbackTheme) {
  ...
  const background = resolveGhosttyColor(config.values.get("background"), (_a = fallbackTheme == null ? void 0 : fallbackTheme.background) != null ? _a : "#000000", void 0);
  const foreground = resolveGhosttyColor(config.values.get("foreground"), (_b = fallbackTheme == null ? void 0 : fallbackTheme.foreground) != null ? _b : "#ffffff", {
    background,
    foreground: (_c = fallbackTheme == null ? void 0 : fallbackTheme.foreground) != null ? _c : "#ffffff"
  });
  ...
  const selectionBackground = resolveGhosttyColor(
    ...
    (_d = fallbackTheme == null ? void 0 : fallbackTheme.selectionBackground) != null ? _d : background,
  ...
  return {
    ...
    black: (_e = palette.get(0)) != null ? _e : fallbackPaletteColor(0, fallbackTheme),
    red: (_f = palette.get(1)) != null ? _f : fallbackPaletteColor(1, fallbackTheme),
    green: (_g = palette.get(2)) != null ? _g : fallbackPaletteColor(2, fallbackTheme),
    yellow: (_h = palette.get(3)) != null ? _h : fallbackPaletteColor(3, fallbackTheme),
    blue: (_i = palette.get(4)) != null ? _i : fallbackPaletteColor(4, fallbackTheme),
    magenta: (_j = palette.get(5)) != null ? _j : fallbackPaletteColor(5, fallbackTheme),
    cyan: (_k = palette.get(6)) != null ? _k : fallbackPaletteColor(6, fallbackTheme),
    white: (_l = palette.get(7)) != null ? _l : fallbackPaletteColor(7, fallbackTheme),
    brightBlack: (_m = palette.get(8)) != null ? _m : fallbackPaletteColor(8, fallbackTheme),
    brightRed: (_n = palette.get(9)) != null ? _n : fallbackPaletteColor(9, fallbackTheme),
    brightGreen: (_o = palette.get(10)) != null ? _o : fallbackPaletteColor(10, fallbackTheme),
    brightYellow: (_p = palette.get(11)) != null ? _p : fallbackPaletteColor(11, fallbackTheme),
    brightBlue: (_q = palette.get(12)) != null ? _q : fallbackPaletteColor(12, fallbackTheme),
    brightMagenta: (_r = palette.get(13)) != null ? _r : fallbackPaletteColor(13, fallbackTheme),
    brightCyan: (_s = palette.get(14)) != null ? _s : fallbackPaletteColor(14, fallbackTheme),
    brightWhite: (_t = palette.get(15)) != null ? _t : fallbackPaletteColor(15, fallbackTheme),
...
function resolveGhosttyColor(value, fallback, context) {
  var _a, _b;
  if (!value) {
    return fallback;
  }
  ...
  if (normalized === "cell-background") {
    return (_a = context == null ? void 0 : context.background) != null ? _a : fallback;
  }
  if (normalized === "cell-foreground") {
    return (_b = context == null ? void 0 : context.foreground) != null ? _b : fallback;
  }
  if (normalized === "none") {
    return "transparent";
  }
...
function buildExtendedAnsiColors(palette) {
  ...
  return extended.length > 0 ? extended : void 0;
}
function fallbackPaletteColor(index, fallbackTheme) {
  if (fallbackTheme) {
    switch (index) {
      case 0:
...
function getParsedFallbackTheme() {
...
var TerminalSettingTab = class extends import_obsidian3.PluginSettingTab {
  ...
  display() {
    ...
    containerEl.empty();
    this.isChinese = (0, import_obsidian3.getLanguage)().toLowerCase().startsWith("zh");
  ...
  renderThemeSelector(containerEl) {
    new import_obsidian3.Setting(containerEl).setName(this.t("theme")).setDesc(this.t("themeDesc")).addDropdown((dropdown) => {
      ...
      const selectedTheme = themes.some((theme) => theme.name === this.plugin.settings.themeName) ? this.plugin.settings.themeName : themes[0].name;
  ...
  renderThemeDirectory(containerEl) {
    const themeDir = this.plugin.getThemeDirectoryPath();
    new import_obsidian3.Setting(containerEl).setName(this.t("themeDirectory")).setDesc(themeDir).addExtraButton((button) => {
      button.setIcon("refresh-cw").setTooltip(this.t("reloadThemes")).onClick(async () => {
        await this.plugin.reloadBundledThemes();
        this.plugin.applyThemeToAllTerminals();
        this.display();
      });
    }).addButton((button) => {
      button.setButtonText(this.t("open")).onClick(async () => {
        const { shell } = require("electron");
        await shell.openPath(themeDir);
      });
    });
  }
  renderThemeInfo(containerEl) {
  ...
  renderAppearanceSettings(containerEl) {
    new import_obsidian3.Setting(containerEl).setName(this.t("fontSize")).setDesc(this.t("fontSizeDesc")).addSlider((slider) => {
      slider.setLimits(10, 24, 1).setValue(this.plugin.settings.fontSize).setDynamicTooltip().onChange(async (value) => {
    ...
    new import_obsidian3.Setting(containerEl).setName(this.t("autocompleteTrigger")).setDesc(this.t("autocompleteTriggerDesc")).addText((text) => {
      text.setPlaceholder(DEFAULT_SETTINGS.autocompleteTrigger).setValue(this.plugin.settings.autocompleteTrigger).onChange(async (value) => {
        const normalized = this.normalizeAutocompleteTrigger(value);
        if (normalized === this.plugin.settings.autocompleteTrigger)
          return;
        this.plugin.settings.autocompleteTrigger = normalized;
        await this.plugin.saveSettings();
      });
    });
    const fontSetting = new import_obsidian3.Setting(containerEl).setName(this.t("fontFamily")).setDesc(this.t("fontFamilyDesc")).addButton((button) => {
      button.setButtonText(this.t("chooseFont")).onClick(async () => {
      ...
      button.setIcon("refresh-cw").setTooltip(this.t("reloadFonts")).onClick(async () => {
        await this.renderFontFamilyControl(fontSetting, true);
      });
    });
    void this.renderFontFamilyControl(fontSetting, false);
  }
  async renderFontFamilyControl(setting, forceRefresh) {
    var _a, _b;
    (_a = setting.infoEl.querySelector(".xterm-font-family-value")) == null ? void 0 : _a.remove();
    (_b = setting.infoEl.querySelector(".xterm-font-status")) == null ? void 0 : _b.remove();
  ...
  normalizeAutocompleteTrigger(value) {
    const trimmed = value.trim();
...
var FontFamilySuggestModal = class extends import_obsidian3.FuzzySuggestModal {
  constructor(app, fonts, currentFont, options) {
    super(app);
    this.fonts = fonts;
...
var XTermTerminalPlugin = class extends import_obsidian4.Plugin {
  constructor() {
    super(...arguments);
  ...
  async loadSettings() {
    ...
    this.settings = {
      ...
      autocompleteTrigger: typeof rawData.autocompleteTrigger === "string" && rawData.autocompleteTrigger.trim().length > 0 ? rawData.autocompleteTrigger.trim() : DEFAULT_SETTINGS.autocompleteTrigger
    };
  ...
  getThemeDirectoryPath() {
    ...
    const pluginDir = manifestDir ? path3.isAbsolute(manifestDir) ? manifestDir : path3.join(vaultPath, manifestDir) : path3.join(vaultPath, ".obsidian", "plugins", this.manifest.id);
    return path3.join(pluginDir, "themes");
  }
  async getInstalledFonts(forceRefresh = false) {
  ...
  async loadInstalledFonts() {
    ...
    try {
      ...
      for (const font of fonts) {
        const family = (_a = font.family) == null ? void 0 : _a.trim();
        if (!family)
          continue;
        const aliases = (_b = fontFamilies.get(family)) != null ? _b : /* @__PURE__ */ new Set();
        const fullName = (_c = font.fullName) == null ? void 0 : _c.trim();
        const postscriptName = (_d = font.postscriptName) == null ? void 0 : _d.trim();
        if (fullName && fullName !== family)
          aliases.add(fullName);
        if (postscriptName && postscriptName !== family && postscriptName !== fullName)
          aliases.add(postscriptName);
        fontFamilies.set(family, aliases);
      }
      return this.withCurrentFont(
        Array.from(fontFamilies.entries()).map(([family, aliases]) => ({
          family,
  ...
  withCurrentFont(fonts) {
    const currentFont = this.settings.fontFamily.trim();
    if (!currentFont)
      return fonts;
    return fonts.some((font) => font.family === currentFont) ? fonts : [{ family: currentFont, aliases: [] }, ...fonts];
  }
  /**
